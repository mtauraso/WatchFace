...
Windows simulated implementation based off of
...
Released to us under th MIT license
*/
/////////////////////////////////////////////////////////////////
...
PCF8563_Class::PCF8563_Class(void)
{
    offset_sec = 0;
}
    ...
    uint8_t second)
{
    ...
    time_t set_epoch = mktime(&set_time);
    time_t now_epoch = time(NULL);

    offset_sec = (int)(set_epoch - now_epoch);
}
...
{
    // TODO: Make an RTC date of the current date (vis-a-vis whatever offset we have)
    // return it.
    time_t now_epoch = time(NULL);
    time_t ret_time = now_epoch + offset_sec;
    struct tm ret_tm;
    localtime_s(&ret_tm, &ret_time);
    return RTC_Date(
        ret_tm.tm_year + 1900,
        ret_tm.tm_mon + 1,
        ret_tm.tm_mday,
...
// On the watch this calles settimeofday to take whatever is in the RTC and sync it into the
// OS time abstraction. On the Simulator we don't allow this because we would have to reimplement time.h
// Instead of fetching the system clock like on the watch and storing it into the RTC, 
// we'll set our stored offset to zero so we return the same as the system clock on the computer
void PCF8563_Class::syncToSystem()
{
    // If this gets called on the sim something bad has happened
    throw std::runtime_error("PCF8563_Class: syncToSystem() Unimplemented. Simulator cannot set a system clock");
}

// On the watch this looks up the current time and stores it into the RTC.
// This works the same 
